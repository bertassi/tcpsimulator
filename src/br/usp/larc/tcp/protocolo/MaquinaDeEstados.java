...
public class MaquinaDeEstados {
    ...
    /**
     * Atributo que guarda o nmero de retransmisses de pacote que foram
     * realizadas
     */
    ...
    /**
     * Objeto Timer que faz a contagem do tempo para ser verificado o tempo de
     * timeout
     */
    private Timer oTimer = null;

    /**
     * Objeto que a tarefa a ser executado quando o tempo de timeout for
     * excedido
     */
    ...
    /**
     * Próximo numero de sequência a ser enviado. Usado no controle de
     * sequência.
     */
    private long proxNumSeq = 0;

    /**
     * Numero de sequência esperado (número do ACK esperado). Usado no controle
     * de sequência.
     */
    private int numAck = 0;

    /**
     * Ultimo número de sequência não confirmado. Usado pelo transmissor no
     * controle de sequência para guardar o último número de sequência
     * transmitido e não confirmado pelo receptor.
     */
    private int numSeqNaoConfirmado = 0;

    /**
     * Tamanho da janela de recepcao. O usuário pode mudar manualmente o valor
     * dessa janela, mas por default vamos utilizar o valor de 100 bytes
     */
    private int tamJanelaRecepcao = 12;

    /**
     * Tamanho da janela de recepcao do receptor remoto. Por default, coloquei
     * 100 bytes, mas na verdade o receptor avisa o valor dessa varivavel
     * atraves do campo janela de recepcao do cabecalho TCP
     */
    private int tamJanelaRemota = this.tamJanelaRecepcao;

    /**
     * Maximum Segment Size. Coloquei esse valor apenar para haver segmentação
     * na hora de transmitir dados. Mas poderia ser outro valors
     */
    private int MSS = 34;
...
//    private byte[] bufferRX = new byte[2 * this.tamJanelaRecepcao];
    ...
    /**
     * Ponteiro para o último byte do buffer de trasmissao que recebeu um ACK
     * (que foi reconhecido)
     */
    private int numSeqTXReconhecida = -1;

    /**
     * Guarda o valor do ultimo byte transmitido na ultima transmissao. Nao tem
     * aplicacao direta no protocolo, e apenas uma variavel auxiliar no processo
    ...
    /**
     * Numero de bytes da mensagem de transmissao que ja foram transmitidos para
     * o buffer. Serve como ponteiro para a cadeia de bytes da mensagem
     */
...
//    private byte[] dadosSegmentoTX;
    /**
     * Numero do ultimo byte da cadeia de dados lido do buffer para o a camada
     * de aplicacao. Usado pela maquina de recepcao de dados
     */
    private int ultimoByteLido = -1;

    /**
     * Numero do ultimo bytena cadeia de dados que chegou da rede e foi colocado
     * no buffer de recepcao. Usado pela maquina de recepcao de dados
     */
    private int ultimoByteRecebido = -1;

    /**
     * Guarda o valor do ultimo byte recebido na ultima recepcao. Nao tem
     * aplicacao direta no protocolo, e apenas uma variavel auxiliar no processo
    ...
    /**
     * Indica se o timeout que ocorreu é de conexao ou de transmissao Se true, o
     * timeout é de transmissao Se false, o timeout é de conexão
     */
    ...
    /**
     * Método que recebe primitivas e executa as operaes para atender a ao. As
     * primitivas esto definidas na interface TCPIF.
     *
     * @param _primitiva A primitiva que enviada.
     * @param args[] Um array de argumentos que voc pode receber adicionalmente
     * @exception Exception Caso ocorra algum erro ou exceo, lana (throw) para
     * quem chamou o mtodo.
     */
...
//    	pacoteDeEnvio.setNumAck(0);
    ...
    /**
     * Mtodo que trata um timeout, escalonando uma tarefa que cuidar do evento
     * de timeout caso o tempo de time out seja excedido
     *
     */
...
//    	}
    ...
    public class TimeOutTask extends TimerTask {
        ...
        /**
         * Este e o metodo ativado quando a task e executada, de acordo com o
         * schedule que foi feito
         */
        public void run() {
            try {
                //this.me.oTimer = null;
    ...
    /**
     * Este metodo e usado no controle de sequencializao, no contexto de envio
     * de segmentos, para uma coisa:<br/>
     * 1. Criar a String que deve ser mostrada no campo segmento no diagrama de
     * tempos da MaquinaDeEstadosFrame
     *
     * @param tipoSegmento O tipo do segmento, ex.: SYN, ACK, SYN+ACK. Este tipo
     * deve ser representado usando as constantes da interface TCPIF
     * @param pacote O pacote TCP que e passado para o metodo. Usado para tirar
     * algumas informacoes do pacote, como por exemplo, tamanho
     * @return Retorna uma String formatada para ser apresentada no campo
     * Segmento do diagrama de tempos da MaquinaDeEstadosFrame
     */
    ...
    /**
     * Este metodo e usado no controle de sequencializao, no contexto de
     * recepcao de segmentos, para uma coisa:<br/>
     * 1. Criar a String que deve ser mostrada no campo segmento no diagrama de
     * tempos da MaquinaDeEstadosFrame<br/>
     * 2. Atualizar a variável que guarda o número de sequencia esperado (ACK)
     *
     * @param tipoSegmento O tipo do segmento, ex.: SYN, ACK, SYN+ACK. Este tipo
     * deve ser representado usando as constantes da interface TCPIF
     * @param pacoteRecebido Pacote recebido. Usado para extrair informacoes que
     * serao redenrizados no diagrama de tempo
     * @return
     */
    ...
    /**
     * Método que implementa a maquina de estados do protocolo de transmissao de
     * dados sobre segmentos TCP
     *
     */
